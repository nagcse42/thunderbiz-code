...
function validate(): boolean {
  ...
  if (!form.mobile.trim()) {
    errors.mobile = 'Mobile number is required'; ok = false
  } else if (!/^\+?[\d\s\-()]+$/.test(form.mobile.trim()) || digits.length < 7 || digits.length > 15) {
    errors.mobile = 'Enter a valid mobile number'; ok = false
  }
